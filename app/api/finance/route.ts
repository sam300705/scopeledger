...
export async function POST(request:Request){try{
 ...
 const idem=await beginIdempotency(request,wid,u.userId,"finance."+v.action,v);
 ...
 try{
  ...
  if(v.action==="invoice"){
   ...
   const id=crypto.randomUUID(),data={id,change_id:v.change_id,invoiced_amount:c.invoiced_amount+v.amount,paid_amount:c.paid_amount,version:v.version+1};
   const operations=[
   ...
   const completion=completeIdempotency(idem,data,201,true);if(completion)operations.push(completion);
  ...
  if(v.action==="payment"){
   ...
   const id=crypto.randomUUID(),data={id,change_id:v.change_id,invoiced_amount:c.invoiced_amount,paid_amount:c.paid_amount+v.amount,version:v.version+1};
   const operations=[
  ...
  if(v.action==="void_invoice"){
   ...
   const data={id:invoice.id,change_id:v.change_id,invoiced_amount:c.invoiced_amount-invoice.amount,paid_amount:c.paid_amount,version:v.version+1};
   const operations=[
   ...
   const completion=completeIdempotency(idem,data,200,true);if(completion)operations.push(completion);
   const result=await db().batch(operations);
  ...
  const data={id:payment.id,change_id:v.change_id,invoiced_amount:c.invoiced_amount,paid_amount:c.paid_amount-payment.amount,version:v.version+1};
  const operations=[
  ...
  return json(data);
 }catch(e){await releaseIdempotency(idem);throw e;}
}catch(e){return failure(e);}}
