...
export async function GET(request:Request){try{
 ...
 const results=await db().batch([
  db().prepare("SELECT COUNT(*) AS project_count,SUM(CASE WHEN archived_at='' THEN 1 ELSE 0 END) AS active_project_count,COALESCE(SUM(budget),0) AS baseline_value FROM projects WHERE workspace_id=?").bind(wid),
  db().prepare("SELECT COUNT(*) AS change_count,SUM(CASE WHEN status='pending' THEN 1 ELSE 0 END) AS pending_count,COALESCE(SUM(CASE WHEN status='pending' THEN amount ELSE 0 END),0) AS pending_value,COALESCE(SUM(CASE WHEN status IN ('approved','delivered') THEN approved_amount ELSE 0 END),0) AS approved_value,COALESCE(SUM(CASE WHEN status='delivered' THEN delivered_amount ELSE 0 END),0) AS delivered_value,COALESCE(SUM(invoiced_amount),0) AS invoiced_value,COALESCE(SUM(paid_amount),0) AS paid_value,COALESCE(SUM(CASE WHEN status IN ('pending','approved','delivered') THEN minutes ELSE 0 END),0) AS active_minutes,SUM(CASE WHEN status='pending' AND due_date<? THEN 1 ELSE 0 END) AS overdue_count,COALESCE(SUM(CASE WHEN status='pending' AND due_date<? THEN amount ELSE 0 END),0) AS overdue_value FROM changes WHERE workspace_id=?").bind(today,today,wid),
 ...
 return json({
  ...
  metrics:{
   ...
   baseline_value:Number(projects.baseline_value||0),
   ...
   active_minutes:Number(changes.active_minutes||0),
